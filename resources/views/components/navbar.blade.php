...
@php
    $ocultarBusqueda = request()->is('login', 'register', 'password/*', 'auth/*', 'producto/*');
@endphp

<nav class="border-b sticky top-0 bg-white z-50" x-data="{
        searchOpen: false,
    ...
    <div class="max-w-full mx-auto px-4 sm:px-8">
        <div class="flex justify-between h-16 items-center gap-4">
            ...
            @if(!$ocultarBusqueda)
                <div class="hidden sm:flex flex-1 max-w-lg items-center">
                    <div class="relative w-full">
                        <input x-ref="searchInput" x-model="query" @keydown.enter="submit()" type="text"
                        ...
                        {{-- Botón limpiar --}}
                        <button x-show="query.length > 0" x-cloak @click="clear()" type="button"
            ...
            <div class="flex items-center space-x-4">
                ...
                {{-- LUPA MÓVIL (oculta en login/register, detalle de productos) --}}
                @if(!$ocultarBusqueda)
                    <button @click="searchOpen = !searchOpen" class="sm:hidden hover:scale-110 transition"
                        aria-label="Buscar">
                        <svg x-show="!searchOpen" class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                        <svg x-show="searchOpen" x-cloak class="h-6 w-6" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @auth
                    <a href="{{ route('perfil.index') }}" class="flex items-center gap-2 hover:scale-105 transition group"
                        title="Mi perfil">
                        @if(Auth::user()->avatar)
    ...
    {{-- BUSCADOR MÓVIL DESPLEGABLE (oculto en paginas inecesarias) --}}
    @if(!$ocultarBusqueda)
        <div x-show="searchOpen" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div class="relative">
                <input x-model="query" @keydown.enter="submit()" type="text" placeholder="Buscar productos, marcas..."
