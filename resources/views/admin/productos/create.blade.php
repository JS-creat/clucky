...
@section('content')
    <div x-data="createProductoForm(@js(old('variantes', [])), @js($errors->toArray()))" class="bg-gray-100 min-h-screen -m-8 p-8 relative">
        ...
        <div class="fixed top-6 right-6 z-50 w-full max-w-sm"
            ...
            <div class="bg-white rounded-2xl shadow-2xl border-l-4 p-5 flex items-start gap-3"
                ...
                <div class="p-1.5 rounded-full flex-shrink-0" :class="toast.type === 'success' ? 'bg-black' : 'bg-rose-500'">
                ...
                <div class="flex-1">
                    <p class="text-xs font-black uppercase tracking-wide" :class="toast.type === 'success' ? 'text-black' : 'text-rose-600'"
                        x-text="toast.type === 'success' ? 'Listo' : 'Falta algo'"></p>
                    <p class="text-sm font-semibold text-gray-700 mt-0.5" x-text="toast.message"></p>
                </div>
                <button type="button" @click="toast.show = false" class="text-gray-300 hover:text-black transition-colors">
        ...
        <div class="flex items-center gap-4 mb-10">
            ...
            <div>
                <h1 class="text-4xl font-extrabold text-black tracking-tight">Crear Producto</h1>
                <p class="text-sm text-gray-500 font-medium mt-1">Completa la información y las variantes de stock.</p>
            </div>
        </div>

        <form x-ref="productForm" @submit.prevent="submitForm" action="{{ route('admin.productos.store') }}" method="POST"
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white p-8 rounded-[2rem] border-2 border-gray-200 shadow-sm space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.nombre_producto">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.nombre_producto[0]"></span>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.marca">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.marca[0]"></span>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.precio">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.precio[0]"></span>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.precio_oferta">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.precio_oferta[0]"></span>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.id_genero">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.id_genero[0]"></span>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <template x-if="errors.id_categoria">
                                <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.id_categoria[0]"></span>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="descripcion" rows="4" maxlength="1000"
                            ...
                            @input="clearError('descripcion')">{{ old('descripcion') }}</textarea>
                        <template x-if="errors.descripcion">
                            <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.descripcion[0]"></span>
                        </template>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2rem] border-2 border-gray-200 shadow-sm">
                    ...
                    <template x-if="errors.variantes">
                        <div class="mb-4 p-3 bg-rose-50 border-2 border-rose-300 rounded-xl text-rose-600 text-xs font-bold">
                    ...
                    <div class="space-y-4">
                        <template x-for="(variante, index) in variantes" :key="variante.uid">
                            <div class="relative grid grid-cols-1 md:grid-cols-4 gap-3 p-5 rounded-2xl transition-all border-2"
                                :class="isDuplicated(index) ? 'bg-rose-50 border-rose-300' : 'bg-gray-50 border-gray-200'">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="text" :name="`variantes[${index}][talla]`" x-model="variante.talla" required
                                        ...
                                        class="w-full px-4 py-3 rounded-xl border-2 font-bold text-sm outline-none transition-all bg-white"
                                    ...
                                    <template x-if="errors[`variantes.${index}.talla`]">
                                        <span class="text-[10px] font-bold text-rose-500" x-text="errors[`variantes.${index}.talla`][0]"></span>
                                    </template>
                                </div>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="text" :name="`variantes[${index}][color]`" x-model="variante.color" required
                                    ...
                                    <template x-if="errors[`variantes.${index}.color`]">
                                        <span class="text-[10px] font-bold text-rose-500" x-text="errors[`variantes.${index}.color`][0]"></span>
                                    </template>
                                </div>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="number" :name="`variantes[${index}][stock]`" x-model="variante.stock" required min="0"
                                        ...
                                        :class="errors[`variantes.${index}.stock`] ? 'border-rose-300' : 'border-gray-200 focus:border-black'">
                                    <template x-if="errors[`variantes.${index}.stock`]">
                                        <span class="text-[10px] font-bold text-rose-500" x-text="errors[`variantes.${index}.stock`][0]"></span>
                                    </template>
                                </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-8 rounded-[2rem] border-2 border-gray-200 shadow-sm space-y-4">
                    <label class="text-xs font-black uppercase tracking-widest text-gray-500 ml-1">
                        Imagen Principal <span class="text-rose-500">*</span>
                    </label>
                    ...
                    <template x-if="errors.imagen">
                        <span class="text-[11px] font-bold text-rose-500 ml-1" x-text="errors.imagen[0]"></span>
                    </template>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2rem] border-2 border-gray-200 shadow-sm space-y-6">
                    <div class="flex justify-between items-center">
                        <label class="text-xs font-black uppercase tracking-widest text-gray-500 ml-1">Galería de Imágenes</label>
                        <span class="text-xs font-black text-black" x-text="`${galeriaFiles.length} foto(s)`"></span>
                    </div>

                    <div class="grid grid-cols-3 gap-3">
                        <template x-for="(item, index) in galeriaFiles" :key="item.id">
                            <div class="relative aspect-square rounded-xl overflow-hidden border-2 border-gray-200 shadow-sm group">
                    ...
                    <div class="relative w-full py-6 border-2 border-dashed border-gray-300 rounded-2xl hover:border-black hover:bg-gray-50 transition-all text-center">
                        <x-heroicon-o-plus class="w-6 h-6 text-gray-500 mx-auto mb-1" />
                        <span class="text-[10px] font-black text-gray-500 uppercase">Añadir fotos a la galería</span>
                ...
                <div class="flex flex-col gap-4">
                    <button type="submit" :disabled="hasErrors() || submitting"
                        ...
                        <span x-text="submitting ? 'Guardando...' : (hasErrors() ? 'Corrige los errores' : 'Crear Producto')"></span>
                    </button>
                    <a href="{{ route('admin.productos.index') }}"
        ...
        <script>
            function createProductoForm(initialVariantes = [], initialErrors = {}) {
                return {
                    variantes: [],
                    ...
                    async submitForm() {
                        ...
                        try {
                            ...
                            if (response.ok) {
                                this.showToast('success', data.message ?? 'Guardado correctamente.');
