            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        ...
                        <th class="px-8 py-6 text-xs font-black uppercase tracking-widest text-gray-400 text-center">Stock</th>
                    ...
                    <tr class="group hover:bg-indigo-50/30 transition-colors">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="font-bold text-gray-900 text-lg leading-tight">{{ $producto->nombre_producto }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-6">
                            <div class="flex justify-end gap-2">
                                ...
                                {{-- Lógica de validación de Stock en el click --}}
                                <button @click="if({{ $producto->stock }} > 0) { errorStockModal = true } else { deleteModal = true; activeId = {{ $producto->id_producto }} }"
            ...
            <div class="relative bg-white rounded-[3rem] p-10 max-w-sm w-full shadow-2xl text-center">
                ...
                <div class="flex flex-col gap-3">
                    <form :action="'{{ route('admin.productos.index') }}/' + activeId" method="POST">
                        @csrf @method('DELETE')
                        <button type="submit" class="w-full py-4 bg-gray-900 text-white font-bold rounded-2xl hover:bg-black transition">Eliminar ahora</button>
                    </form>
                    <button @click="deleteModal = false" class="w-full py-4 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition">Cancelar</button>
                </div>
            </div>
        </div>
    </template>

    {{-- MODAL 2: Alerta de Stock (No se puede eliminar) --}}
