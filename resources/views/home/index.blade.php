...
@section('categorias')
    ...
    <div class="sticky top-16 bg-white border-b z-40 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-8">
            <div
                class="flex justify-center space-x-4 sm:space-x-8 py-3 font-medium uppercase text-xs sm:text-sm tracking-widest overflow-x-auto">
                <a href="{{ route('home') }}"
            ...
            <div x-data="{
                ...
                <div class="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-lg bg-white [aspect-ratio:4/3] sm:[aspect-ratio:21/7]"
                    ...
                    @foreach($banners as $index => $banner)
                        <div x-show="current === {{ $index }}" x-transition:enter="transition-opacity duration-700 ease-in-out"
                            ...
                            <img src="{{ asset('banners/' . $banner->imagen) }}" alt="{{ $banner->titulo }}"
                                class="w-full h-full object-cover object-center" loading="{{ $index === 0 ? 'eager' : 'lazy' }}">
                            @if($banner->titulo || $banner->texto_boton)
                                <div class="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent"></div>

                                <div class="absolute inset-0 flex items-center px-5 sm:px-14" x-show="current === {{ $index }}"
                                    ...
                                    <div class="text-white max-w-[60%] sm:max-w-xs lg:max-w-sm">
                                        @if($banner->etiqueta)
                                            <p
                                                class="text-[9px] sm:text-[11px] font-bold uppercase tracking-[0.25em] sm:tracking-[0.35em] text-white/65 mb-1 sm:mb-1.5">
                                                {{ $banner->etiqueta }}
                                            </p>
                                        @endif
                                        ...
                                        @if($banner->texto_boton && $banner->url_boton)
                                            <a href="{{ $banner->url_boton }}"
                                                ...
                                                {{ $banner->texto_boton }}
                                                <svg class="w-2.5 h-2.5 transition-transform group-hover:translate-x-1" fill="none"
                    ...
                    @if($banners->count() > 1)
                        <button @click="prev()" aria-label="Anterior"
                            ...
                            <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" stroke-width="2.5"
                                viewBox="0 0 24 24">
        ...
        <section class="w-full bg-gray-50 py-3 sm:py-6">
            <div class="max-w-6xl mx-auto px-3 sm:px-8">
                <div class="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-lg [aspect-ratio:4/3] sm:[aspect-ratio:21/8]"
                    ...
                    <img src="{{ asset('images/banner-home.jpg') }}" alt="Banner"
                        class="absolute inset-0 w-full h-full object-cover">
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent flex items-center px-6 sm:px-10">
                        <div class="text-white">
                            <p class="text-[9px] sm:text-[11px] font-bold uppercase tracking-widest text-white/65 mb-1">Nueva
                                Colección 2026</p>
                    ...
                    <a href="{{ url('/producto/' . $item->id_producto) }}"
                        ...
                        <div class="relative aspect-[3/4] bg-gray-50 overflow-hidden">
                            @if($item->precio_oferta)
                                <span
                                    class="absolute top-2 left-2 bg-red-500 text-white text-[8px] sm:text-[9px] font-black px-2 py-0.5 sm:px-2.5 sm:py-1 z-10 uppercase tracking-wider rounded-full shadow-sm">
                                    OFERTA
                                </span>
                            @endif
                        ...
                        <div class="p-2.5 sm:p-3.5 space-y-0.5 sm:space-y-1">
                            <p class="text-[8px] sm:text-[9px] text-gray-400 uppercase font-bold tracking-widest">{{ $item->marca }}
                            </p>
                            <h3 class="text-[12px] sm:text-[13px] font-semibold text-gray-800 leading-snug line-clamp-2">
                                {{ $item->nombre_producto }}</h3>
                            <div class="h-px bg-gray-100 my-1.5 sm:my-2"></div>
                            <div class="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                                @if($item->precio_oferta)
                                    <span class="text-xs sm:text-sm font-black text-red-500">S/
                                        {{ number_format($item->precio_oferta, 2) }}</span>
                                    <span class="text-[10px] sm:text-[11px] text-gray-400 line-through">S/
                                        {{ number_format($item->precio, 2) }}</span>
                                    <span
                                        class="ml-auto text-[8px] sm:text-[9px] font-bold text-red-500 bg-red-50 px-1 sm:px-1.5 py-0.5 rounded">
                                        -{{ round((1 - $item->precio_oferta / $item->precio) * 100) }}%
                                    </span>
                                @else
                                    <span class="text-xs sm:text-sm font-bold text-gray-900">S/
                                        {{ number_format($item->precio, 2) }}</span>
                                @endif
                            </div>
                        </div>
